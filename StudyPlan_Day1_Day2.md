...
> _Example:_ A `BankAccount` class keeps `balance` private. Outside code can only call `Deposit()` or `Withdraw()` — it cannot directly set `balance = -1000`.
...
> _Example:_ When you call `dbContext.SaveChangesAsync()`, you don't need to know that it generates SQL, tracks changes, handles concurrency. You just call it.
...
> _Example:_ `EmployeeController : ControllerBase` — your controller inherits all HTTP response helper methods (`Ok()`, `NotFound()`, `BadRequest()`) from `ControllerBase`.
...
> _Example:_ `Shape.Draw()` behaves differently when the actual object is a `Circle` vs a `Square`. The calling code doesn't need to know the type — it just calls `Draw()`.
...
> _Why important?_ Almost all LINQ is written with lambdas: `employees.Where(e => e.Salary > 50000)`. Understanding lambdas = understanding LINQ.
...
> _Use cases:_ Logging request/response, validating custom headers, measuring execution time, caching responses.
...
| ------------------ | -------------------------------------- | ------------------------------ |
| **Authentication** | _Who are you?_ — Verify identity       | JWT token, Cookie, API Key     |
| **Authorization**  | _What can you do?_ — Check permissions | `[Authorize]`, Roles, Policies |
